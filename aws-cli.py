...
class botohelper:
    ...
    def stop(self, stp):
        try:
            response = client.stop_instances(InstanceIds=[stp], DryRun=False)
        except ClientError as e:
            print(e)
            if 'DryRunOperation' not in str(e):
                raise

    def terminate(self, tmt):
        try:
            response = client.terminate_instances(InstanceIds =[tmt], DryRun=False)
        except ClientError as e:
            print(e)
            if 'DryRunOperation' not in str(e):
                raise
...
def showa(bot,update):
    for i in range(len(arr)):
        q, l = arr[i][0], arr[i][1]
        if(q!='terminated'):
            bot.send_message(chat_id=abcdef, text=q + ' : ' + l)
